...
class MisPedidos extends Component {
  ...
  async componentDidMount() {
    this._isMounted = false;
    const thisa=this;
     firebase.firestore().collection("pedido").onSnapshot(function(querySnapshot) {
      ...
      <Redirect to="/products" />
        ...
        <div className="hero is-primarys">
          <div className="hero-body container">
            <h1 className="title tamleta">Pedidos</h1>
          </div>
        </div>
        <br />
        ...
        <div className="containerc">
          {pedidos.length ? (
            ...
            <div className="column">
              <div className="title has-text-grey-light tamleta">No hay pedidos!</div>
            </div>
          )}
        </div>
      </> );
...
export default withContext(MisPedidos);